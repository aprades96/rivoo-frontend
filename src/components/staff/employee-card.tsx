...
/**
 * Badge de estado del empleado (D9): compartido por la tarjeta movil y la
 ...
 * (`border --border`, `bg --card`, `--muted-foreground-2`), que tambien vale
 * para el movil segun la tabla de D9 ("igual").
 */
...
/**
 * Fila de empleado en movil (`Equipo.dc.html:58`). Pasa a `<Link>` (D5): la
 ...
 * D5): el artboard movil no lo dibuja, a diferencia de la fila de escritorio.
 *
 * D9: en movil la fila inactiva NO cambia de fondo (`Equipo.dc.html:94` no la
 * tinta) -- solo el nombre y el puesto atenuan su color.
 */
export function EmployeeCard({ employee }: EmployeeCardProps) {
  return (
    <Link
      href={`/staff/${employee.id}`}
      className="flex items-center gap-3 rounded-lg border border-border bg-card p-3 outline-none transition-colors hover:bg-muted/50 focus-visible:ring-2 focus-visible:ring-ring"
    >
      <Avatar className="h-10 w-10">
        <AvatarFallback className="text-sm font-semibold" style={employeeAvatarStyle(employee.colorHex)}>
